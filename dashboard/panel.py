...
Cung cấp 4 sub-tabs:
  - Chat: gọi /api/chat với view latency breakdown + validation gates
  - Ingest: upload file → /api/ingest/upload với multi-format support
  - Health: /api/health/deep — component + dependencies + metrics
  - Community: trigger /api/community/build
...
def chat_fn(api_base: str, query: str, tenant_id: str, format_filter: str, max_retries: int):
    if not query.strip():
...
def ingest_fn(api_base: str, file, tenant_id: str, access_level: str, department: str, author: str):
    if file is None:
    ...
    try:
        ...
        if author:
            data["author"] = author
        result = _post_files(f"{api_base}/api/ingest/upload", files, data, timeout=600.0)
    except Exception as e:
        return f"**Lỗi**: {e}", ""
...
def health_fn(api_base: str):
    try:
        data = _get(f"{api_base}/api/health/deep", timeout=15.0)
    except Exception as e:
        return f"**Lỗi**: {e}", "", "", ""

    status_md = (
        f"## Status: `{data.get('status')}`\n\nPipeline enabled: **{data.get('pipeline_enabled')}**"
    )
    ...
    metrics_md = (
        f"**Chats total**: {metrics.get('pipeline_chats_total', 0)}\n"
        f"**Refusal rate**: {metrics.get('pipeline_refusal_rate', 0):.2%}\n"
        f"**Validation pass rate**: {metrics.get('pipeline_validation_pass_rate', 0):.2%}\n"
        f"**Avg grounded ratio**: {metrics.get('pipeline_avg_grounded_ratio', 0):.3f}\n"
        f"**Avg consistency**: {metrics.get('pipeline_avg_consistency_score', 0):.3f}\n"
        f"**Communities built**: {metrics.get('pipeline_communities_built', 0)}"
    )
...
def community_fn(
    ...
    try:
        result = _post_json(
            f"{api_base}/api/community/build",
            {
                "tenant_id": tenant_id or "default",
                "levels": int(levels),
...
def build_vrag_tab(api_base: str = _DEFAULT_API) -> None:
    ...
    gr.Markdown(
        ...
        "Endpoints: `/api/chat`, `/api/ingest/upload`, `/api/community/build`, `/api/health/deep`.\n"
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("Chat"):
            ...
            chat_btn.click(
                fn=lambda q, t, f, r: chat_fn(api_base, q, t, f, r),
        ...
        with gr.TabItem("Ingest"):
            gr.Markdown(
                "### Upload tài liệu đa định dạng qua VRAG pipeline\n"
            ...
            ingest_btn.click(
                fn=lambda f, t, a, d, au: ingest_fn(api_base, f, t, a, d, au),
        ...
        with gr.TabItem("Health"):
            gr.Markdown("### Deep health check VRAG pipeline")
        ...
        with gr.TabItem("Community Build"):
            ...
            comm_btn.click(
                fn=lambda t, l, r, m, v: community_fn(api_base, t, l, r, m, v),
