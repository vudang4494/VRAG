...
async def aggregate_entity_embedding(
    ...
    """
    # Tenant filter must bind to the variable in scope at each WHERE: `e` is bound by
    # the first MATCH, `c` only by the second. The old code put "AND c.tenant_id = $tid"
    # on BOTH WHEREs — the first referenced `c` before it existed, so every call raised a
    # Cypher error that the try/except below swallowed, leaving the GAEA entity cache and
    # the HEFR entity collection permanently empty.
    where_e = "AND e.tenant_id = $tid" if tenant_id else ""
    where_c = "AND c.tenant_id = $tid" if tenant_id else ""
    cypher = f"""
    MATCH (e:Entity)
    WHERE toLower(e.name) = toLower($name) {where_e}
    MATCH (c:Chunk)-[:CONTAINS_ENTITY]->(e)
    WHERE c.id IS NOT NULL {where_c}
