...
async def generate_views(
    ...
    # Trim 4→2 views: paraphrase + summary cover the diversity needed for RRF.
    # question + keywords add ~50% latency at ingest for marginal recall lift
    # (verified Test D run with all views OFF tanked validation by 12pp;
    # re-enabling 2 views recovers most of that at 2× ingest cost).
    tasks = {
...
async def embed_views(
    ...
    """Embed mỗi view. Nếu embed_prefix có, prepend vào mọi view text BEFORE
    embedding (doc-context binding). View text gốc trong payload không đổi —
    enrichment chỉ ảnh hưởng vector, không ảnh hưởng citation hiển thị."""
    names = list(views.keys())
    texts = [(embed_prefix + views[n]) if embed_prefix else views[n] for n in names]
...
async def process_chunk_consistency(
    ...
    embeds = await embed_views(views, http, embed_url, embed_model, embed_prefix=embed_prefix)
    ...
    async def _one(chunk: dict) -> dict:
        async with sem:
            result = await process_chunk_consistency(
                ...
                llm_model,
                enable_llm_views,
                embed_prefix=chunk.get("embed_prefix", ""),
            )
            return {**chunk, **result}
