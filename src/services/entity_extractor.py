...
# NOTE (2026-07-16): `concept` + `event` were dropped from the default set. On
# real chunk text they were the dominant NOISE source in the KG (~40-50% junk):
# GLiNER tags abstract nouns and generic phrases ("onboarding process", "the
# meeting", "data protection") as concept/event, which pollute Entity nodes and
# every downstream graph path. Named-entity types (person/org/location/product/
# technology/date) are what a KG should pivot on. Restore the noisy labels per
# tenant via ENTITY_EXTRACTOR_LABELS if a corpus genuinely needs them.
DEFAULT_LABELS = [
    "person",
    "organization",
    "location",
...
# Names GLiNER sometimes emits as person/org but that carry no graph value —
# pronouns and bare determiners. Filtered at the extraction choke-point.
...
def clean_entity_name(raw: str) -> str:
    """Normalize a raw GLiNER span into a KG-safe name.

    Collapses internal whitespace/newlines ("Human\\nResources" → "Human
    Resources" — a real artifact seen in ingested PDFs) and strips wrapping
    quotes/punctuation. Returns "" for spans that should be dropped.
    """
...
class GLiNERExtractor(BaseEntityExtractor):
    ...
    async def extract(self, text: str) -> tuple[list[ExtractedEntity], list[ExtractedRelation]]:
        ...
        for item in raw:
            name = clean_entity_name(item.get("text") or "")
            lbl = (item.get("label") or "").lower()
            score = float(item.get("score", 0.0))
            if len(name) < 2 or score < self.threshold:
                continue
            key = name.lower()
            # Drop bare pronouns/determiners ("you", "they", "this") — but NOT
            # all-caps acronyms that happen to collide with one ("ITS" = IT
            # Services, "WHO" = World Health Org). Pronouns arrive lower/title-case.
            if key in _STOP_NAMES and not name.isupper():
                continue
