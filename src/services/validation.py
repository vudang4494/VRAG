...
async def hallucination_gate(
    ...
    Perf: cap claims at `max_claims` to bound verification cost. Most VN
    answers have 2-4 atomic claims; capping at 5 keeps cost bounded for
    rare long answers. concurrent_limit=8 lets Ollama batch verifies.
    """
    ...
    sem = asyncio.Semaphore(
        5
    )  # 5 = matches rerank stage pattern; 8 can oversaturate Ollama on M4 Metal
...
_CITE_STRIP = re.compile(r"\[[\w\-:]+\]")


async def grounding_gate_cosine(
    answer: str,
    passages: list[str],
    http: Any,
    embed_url: str,
    embed_model: str,
    min_grounded_ratio: float = 0.70,
    sim_hi: float = 0.60,
    sim_lo: float = 0.40,
    max_sentences: int = 12,
) -> dict[str, Any]:
    """Grounding via embedding cosine — deterministic, no LLM.

    Replaces the LLM extract-claims + verify-each path (up to 6 gemma calls,
    ~25s/answer measured) with one bge-m3 embed batch (~0.7s). Split the answer
    into sentences, embed sentences + retrieved chunks, score each sentence by its
    max cosine to any chunk:
      >= sim_hi → fully grounded (1.0)
      <= sim_lo → ungrounded (0.0)
      between   → linear partial credit
    grounded_ratio = mean credit. Thresholds calibrated on the live stack: grounded
    sentences score 0.72-0.93, off-topic/hallucinated 0.30-0.35 (see config docstring).
    """
    from src.services.embedding import cosine_similarity, embed_batch

    sents = [
        _CITE_STRIP.sub("", s).strip()
        for s in re.split(r"(?<=[.!?])\s+", answer)
        if len(_CITE_STRIP.sub("", s).strip()) > 10
    ]
    if not sents:
        return {"passed": True, "grounded_ratio": 1.0, "claims_total": 0, "verdicts": []}
    if not passages:
        # No context to ground against — treat as ungrounded (mirrors LLM path failing).
        return {"passed": False, "grounded_ratio": 0.0, "claims_total": len(sents), "verdicts": []}
    ...
    total = 0.0
    for s, sv in zip(sents, sent_vecs, strict=False):
        mx = max((cosine_similarity(sv, cv) for cv in chunk_vecs), default=0.0)
        if mx >= sim_hi:
            credit, label = 1.0, "YES"
        elif mx <= sim_lo:
            credit, label = 0.0, "NO"
        else:
            credit = (mx - sim_lo) / (sim_hi - sim_lo)
            label = "PARTIAL"
        total += credit
        verdicts.append((s, f"{label}({mx:.2f})"))

    ratio = total / len(sents)
    return {
        "passed": ratio >= min_grounded_ratio,
        "grounded_ratio": ratio,
        "claims_total": len(sents),
        "verdicts": verdicts,
    }
...
async def entity_gate(
    ...
    try:
        ...
        # Empty-KG guard: with no entities to check against (e.g. a dense-only
        # corpus where GLiNER was off), this gate would flag EVERY proper noun as
        # invalid → spurious refusal + a full regeneration. Nothing to validate ⇒ pass.
        if not exact_matches and not kg_names:
            return {
                "passed": True,
                "invalid": [],
...
async def validate_answer(
    ...
    embed_url: str = "",
    embed_model: str = "",
    use_cosine_grounding: bool = True,
    grounding_sim_hi: float = 0.60,
    grounding_sim_lo: float = 0.40,
) -> dict[str, Any]:
    """
    Run 3 gates in parallel. Return combined result.

    Grounding gate: cosine (deterministic, needs `passages` + embed client) by
    default; falls back to the LLM claim-verify gate when cosine is disabled or the
    embed client / passages are unavailable.
    ...
    """
    if use_cosine_grounding and passages is not None and http is not None:
        halluc_task = grounding_gate_cosine(
            answer,
            passages,
            http,
            embed_url,
