...
class SemanticChunker(BaseChunker):
    ...
    async def _group_by_topic(self, sentences: list[str]) -> list[str]:
        ...
        # F4: pack tiny paragraphs into neighbors so bibliography fragments
        # (e.g. "ArXiv, abs/2308.07107." or "Definition 2.1.") don't become
        # standalone chunks. A paragraph <80 chars gets appended to the
        # previous one regardless of topic shift, unless that would exceed
        # 2× max. First paragraph never absorbs into anything.
        ...
        for p in paragraphs:
            ...
            if (
                merged_paragraphs
                and len(joined) < 80
                and len(" ".join(merged_paragraphs[-1])) + len(joined)
                < self.paragraph_max_chars * 2
            ):
                merged_paragraphs[-1].extend(p)
            else:
                merged_paragraphs.append(p)

        return [" ".join(p) for p in merged_paragraphs]
