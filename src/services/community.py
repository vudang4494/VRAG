...
async def _entity_names_with_labels(
    neo4j_driver, tenant_id: str | None, labels: list[str]
) -> set[str]:
    """Names of this tenant's entities whose type (lowercased) is in `labels`."""
    where_tenant = "AND e.tenant_id = $tid" if tenant_id else ""
    params: dict[str, Any] = {"labels": labels}
    if tenant_id:
        params["tid"] = tenant_id
    async with neo4j_driver.session() as s:
        res = await s.run(
            f"MATCH (e:Entity) WHERE toLower(e.type) IN $labels {where_tenant} "
            "RETURN e.name AS name",
            **params,
        )
        return {row["name"] for row in await res.data()}
...
async def build_communities_for_tenant(
    ...
    """
    Fetch graph → cluster → (optionally summarize) → write back. Clean rebuild
    (this tenant's existing Community nodes are deleted first).

    summarize=False = LazyGraphRAG cluster-only build: writes IN_COMMUNITY
    membership with no LLM summary (summary stays NULL → invisible to the local
    cosine community path); the global-query map-reduce summarizes at query time.
    use_npmi=True clusters on the NPMI de-hub co-occurrence graph (ppr) instead
    of raw RELATES_TO / co-occ edges.
    exclude_labels drops entity types from the graph BEFORE clustering (measured:
    academic corpora are dominated by PERSON author names → one giant blob
    community; see config.community_exclude_labels_csv).
    """
    exclude = sorted({x.strip().lower() for x in (exclude_labels or []) if x.strip()})
    if use_npmi:
        import networkx as nx

        from src.services.ppr import _build_npmi_graph

        g = nx.Graph()
        async with neo4j_driver.session() as _sess:
            await _build_npmi_graph(g, _sess, tenant_id or "default", lambda n: n, npmi_min)
        entity_names = list(g.nodes())
        edges = [(u, v, float(d.get("weight", 1.0))) for u, v, d in g.edges(data=True)]
        name_to_entity = {n: {"name": n} for n in entity_names}
        entities_total = len(entity_names)
        drop = (
            await _entity_names_with_labels(neo4j_driver, tenant_id, exclude) if exclude else set()
        )
    else:
        ...
        drop = (
            {e["name"] for e in entities if (e.get("type") or "").lower() in exclude}
            if exclude
            else set()
        )
    ...
    entities_excluded = 0
    if drop:
        entities_excluded = sum(1 for n in entity_names if n in drop)
        entity_names = [n for n in entity_names if n not in drop]
        edges = [(u, v, w) for (u, v, w) in edges if u not in drop and v not in drop]
    ...
    # Clean rebuild: drop this tenant's existing communities so re-runs don't accumulate.
    async with neo4j_driver.session() as _s:
        await _s.run("MATCH (com:Community {tenant_id: $tid}) DETACH DELETE com", tid=tenant_id)
    ...
    return {
        ...
        "summaries_written": written if summarize else 0,
