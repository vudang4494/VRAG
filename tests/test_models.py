...
# The tag the stack is CONFIGURED to use — never a hardcoded literal. Asserting a
# fixed tag is how this suite came to demand qwen3.5:9b, which is not installed.
...
class TestOllamaServer:
    ...
    def test_configured_model_available(self):
        """The configured generator must exist in Ollama.

        A missing tag does not fail at boot — it 404s one call deep and surfaces as
        "not enough information", i.e. as a retrieval problem. This test is the cheap
        place to catch it.
        """
        names = [m["name"] for m in httpx.get(f"{OLLAMA}/api/tags", timeout=10).json()["models"]]
        assert MODEL in names, f"configured OLLAMA_MODEL={MODEL} not installed: {names}"
