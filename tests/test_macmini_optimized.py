...
# The tag the stack is configured to use — never a hardcoded literal.
...
class TestRAGPipeline:
    ...
    async def test_rag_chat_completions(self, query):
        ...
        async with httpx.AsyncClient(timeout=120) as client:
            ...
            r = await client.post(
                f"{API_BASE}/api/chat",
                json={"query": query, "tenant_id": "eval", "max_retries": 0},
    ...
    async def test_semantic_cache_hit(self):
        ...
        body = {"query": query, "tenant_id": "eval", "max_retries": 0}
        async with httpx.AsyncClient(timeout=120) as client:
            # First call — cache miss
            start1 = time.monotonic()
            r1 = await client.post(f"{API_BASE}/api/chat", json=body)
            latency1 = (time.monotonic() - start1) * 1000
            assert r1.status_code == 200

            # Second call — cache hit (chat_history semantic cache, same tenant+session)
            await asyncio.sleep(0.5)
            start2 = time.monotonic()
            r2 = await client.post(f"{API_BASE}/api/chat", json=body)
            ...
            # Only a real assertion when the first answer passed validation (else no store).
            if not r1.json().get("refused"):
                assert r2.json().get("history_cache", {}).get("hit") is True
