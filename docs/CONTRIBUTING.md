...
1. **VRAG is one product.** No `v1`/`v2`/`v3` in filenames, function names, classes, modules, or branding. The API lives under `/api/*`. If you find leftover version suffixes anywhere else, please file a PR to remove them.
...
7. **Never commit `.env` or files under `.agents/`.** Both are in `.gitignore` for a reason.
...
- [Ollama](https://ollama.ai) on host with `gemma4:e4b` and `bge-m3` pulled (must match `src/config.py`)
...
| `tests/` | pytest |
| `.agents/internal-docs/` | Project-internal docs — **gitignored** |
...
- For algorithmic changes, run `make pipeline-bench` (scripts/benchmark.py) and include the report in the PR description so we can compare before/after
...
python3 scripts/benchmark.py   # self-contained CRAG/multi-hop/VN/OOD cases
```
...
By contributing you agree your work is licensed under [Apache 2.0](../LICENSE). VRAG's attribution requirement applies — see [NOTICE](../NOTICE).
