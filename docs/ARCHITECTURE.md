...
These choices come at a cost: latency is higher than managed RAG. On the shipping `gemma4:e4b` config, measured p95 is ~26s (p50 ~15s) on M-series; sustained multi-user QPS still benefits from GPU. We accept that trade because the alternative — fast but ungrounded — is unacceptable for the enterprise/regulated use cases VRAG targets.
...
| Named vector: `graph_aware` | 1024-d, cosine | BGE-M3 graph-aware re-embed (via `/api/gaea/refine`) |
...
| 3 (Semantic) | Lexical-proposed variants confirmed by entity-centroid cosine → soft-fold `ALIAS_OF` (`kg.py::resolve_entity_aliases`, backfill via `POST /api/entity_resolution/build`) | ✅ Implemented (OFF by default — `ENTITY_RESOLUTION_ENABLED`) |
| 4 (Hard merge) | `apoc.refactor.mergeNodes` for confirmed duplicates | 📋 Roadmap |

The remaining roadmap item is the largest known gap from the original design plan (`.agents/internal-docs/caitien.md` — local only, **gitignored**; not present in a fresh clone).
...
#### Global map-reduce branch (gated)

If the query matches the `global` regex patterns (`query_router.py::_GLOBAL_PATTERNS` — thematic/corpus-wide questions) and `GLOBAL_QUERY_ENABLED=1` (default 0), `api/routes/_chat.py` short-circuits into `src/services/global_query.py::global_map_reduce` instead of the retrieval pipeline: **MAP** — one light-LLM call per community (capped at `GLOBAL_QUERY_MAX_COMMUNITIES`, default 10), dropping `NO_INFO` partials; **REDUCE** — one heavy-LLM call synthesizes the survivors. Communities come from the lazy cluster-only build (`build_communities_for_tenant(summarize=False)`, triggered via `POST /api/community/build`) — no pre-computed summaries needed (LazyGraphRAG style).
...
Streaming endpoint `/api/chat/stream` uses `ollama_chat_stream` to yield tokens via SSE.
...
|---|---|---|
| Hallucination | Cosine grounding (default — `VALIDATION_COSINE_GROUNDING=1`): embed answer sentences vs retrieved chunks (bge-m3, sim_hi=0.60/sim_lo=0.40). Fallback (`=0`): extract claims → LLM verifies each against retrieved context | `grounded_ratio ≥ 0.70` |
| Entity | Extract entities from answer → check existence in Neo4j | `invalid_entities ≤ 3` |
...
| `ppr` | HippoRAG-2 Personalized PageRank over the entity graph (w=1.7) | If entities detected (`ppr_enabled`, default ON) |
| `entity_cosine` | Cross-doc entity-centroid cosine + TF-IDF + MMR (w=1.6) | If `ENTITY_COSINE_ENABLED` |
| `entity_gate` | Primary entity-gate: entity-centroid cosine run first, scopes downstream paths (w=1.8) | If `ENTITY_GATE_ENABLED` |

KG-derived paths (`entity_pivot`, `graph`, `community`, `entity_cosine`, `ppr`, `entity_gate`) have their RRF weights multiplied by `RRF_KG_PATH_WEIGHT_SCALE` (default 0.2) — at 1.0× they net-hurt recall@5; see `retrieval.py::reformulation_weight`.
...
| Entity name fragmentation | 3-Tier Canonicalization (Tier 3 semantic implemented, OFF by default) |
| Stale community summaries | `/api/community/build` re-runs Leiden; summaries regenerated for changed communities |
...
CPU/Metal baseline (M-series Docker, `gemma4:e4b`, all defaults; measured 2026-07-17):
- p50: ~14.8s (LLM gen + validation dominant)
- p95: ~26.3s · max ~28.0s
- Retrieval alone: <1s
- Tier 1 zero-LLM: <500ms hot

GPU projection (single RTX 4090 / A100 / M4 Max Metal):
- Lower single-query latency (LLM gen is the dominant term)
- Throughput: much higher under concurrency (gen no longer single-threaded CPU-bound)
...
`/api/cross_doc/build` runs `link_chunks_cross_doc`:
...
| Balanced (default) | `QUERY_REFORMULATIONS=0 CONTEXT_COMPRESSION_ENABLED=0 RERANK_STAGE1_ENABLED=0 GENERATION_DRAFTS=1 GENERATION_REFINE_ENABLED=0` (these ARE the code defaults; turning compression/refine on is an opt-in trade) |
| Global/thematic questions (map-reduce over communities) | `GLOBAL_QUERY_ENABLED=1` (default 0) + `GLOBAL_QUERY_MAX_COMMUNITIES` (default 10) |
